using System.Collections;
using System.Collections.Generic;
using TMPro;
using UnityEngine;

public class VerticalBlocks : MonoBehaviour
{
    [SerializeField]
    private Transform VerticalBlock; //좌우로 움직이는 블럭

    [SerializeField]
    private float MoveTime; // 움직이는 시간


    [SerializeField]
    private float BlockSpeed; // 블럭 속도

    [SerializeField]
    private Transform Player; //움직이는 캐릭터

    float time;

    public static bool BlockRide;

    bool a, b; //a가 ture일 경우 좌측이동 또는 상승 b는 우측이동 또는 하강함

    void Start()
    {
        a = true;
        b = false;
        BlockRide = false;
        //InvokeRepeating("Reverse", BlockSpeed, BlockSpeed);
    }

    void FixedUpdate()
    {


        if (a)
        {
            VerticalBlock.position += new Vector3(0, 1, 0) * BlockSpeed * Time.deltaTime;
            if (BlockRide)
            {
                Player.position += new Vector3(0, 1, 0) * BlockSpeed * Time.deltaTime;
            }
        }

        if (b)
        {
            VerticalBlock.position += new Vector3(0, -1, 0) * BlockSpeed * Time.deltaTime;
            if (BlockRide)
            {
                Player.position += new Vector3(0, -1, 0) * BlockSpeed * Time.deltaTime;

            }
        }

        time += Time.deltaTime;

        if (time > MoveTime)
        {
            time = 0;
            a = !a;
            b = !b;
        }


    }
    private void OnCollisionEnter2D(Collision2D collision)
    {
        if(collision.gameObject.name == "character")
        {
            BlockRide = true;
            MoveCharacter.CrashUnit = true;
            MoveCharacter.anim.SetBool("jump", false);
            MoveCharacter.anim.SetBool("walk", false);
            MoveCharacter.anim.SetBool("stay", true);
            MoveCharacter.Jumpcount = Status.Jumpcount;
            MoveCharacter.turnjump = 0;
            MoveCharacter.a = false;
            MoveCharacter.b = false;
        }
    }

    private void OnCollisionStay2D(Collision2D collision)
    {
        if (collision.gameObject.name == "character")
        {
            BlockRide = true;
        }
    }

    private void OnCollisionExit2D(Collision2D collision)
    {
        if (collision.gameObject.name == "character")
        {
            BlockRide = false;
        }
    }
}
